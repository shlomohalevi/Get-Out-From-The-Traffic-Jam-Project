...
namespace ProMaterialCombiner {
	...
	public class ShaderManager {
	    ...
	    public List<Texture2D> GetTexturesForObjectsssssssssssss(Material mat, string shaderName/*, bool generateTexturesIfNecessary = false*/) {
	        List<string> defines;
	    ...
	    public List<string> GetShaderTexturesDefines(string shaderToUse, bool includeNullTextures = true, Material objectMaterial = null) {
            ...
            if(shaderInfoCache.ContainsKey(shaderName)) {//if shader is not catched, cache it.
                ...
                Debug.Log("Count: " + count + " include null?: " + includeNullTextures + " objMaterial: " + (objectMaterial ? objectMaterial.name : "null mat"));
                for(int i = 0; i < count; i++) {
                    if(ShaderUtil.GetPropertyType(mat.shader, i) == ShaderUtil.ShaderPropertyType.TexEnv) {

						#if UNITY_5_4_OR_NEWER
						if(ShaderUtil.GetTexDim(mat.shader, i) == UnityEngine.Rendering.TextureDimension.Tex2D) { //ShaderUtil.ShaderPropertyTexDim.TexDim2D) {<- 5.3-
						#else
                ...
                Debug.Log(shaderTextureDefines.Count);
